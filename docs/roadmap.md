...
路线不是固定课程表。Project01 已经确定；其余项目只保留粗粒度方向，在真正开始前根据当时已经掌握的能力重新细化。

## 递进原则

每个项目都必须满足三件事：

1. 做出一个可以运行、可以观察的具体成果。
2. 只引入下一阶段真正需要的新知识。
3. 让上一个项目的经验在下一个项目中被实际复用。

不为了“完整”额外安排报告、模板、框架或与目标无关的技术。

## 路线总览

| 项目 | 最终做出的东西 | 主要新增能力 | 通向下一步的作用 |
|---|---|---|---|
...
学习大量独立状态的连续更新：数据怎么放、CPU 怎么读、编译器能否向量化，以及如何把数值状态画出来。
...
1. 当前项目里哪些能力已经真正掌握？
2. 下一目标还缺哪一层关键能力？
3. 下一个项目怎样在不重复、不跳级的情况下补上这一层？

然后才细化下一个项目。更远项目不提前拆成几十条 checklist。
