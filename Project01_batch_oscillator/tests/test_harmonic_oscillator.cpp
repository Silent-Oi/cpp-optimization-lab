...
void test_harmonic_time_integrators() {
    ...
    expect_state_near(explicit_euler_step(dt, initial_state, system),
                      State{.position = 2.6, .velocity = -5.2}, "explicit Euler step");
    expect_state_near(symplectic_velocity_euler_step(dt, initial_state, system),
                      State{.position = 2.48, .velocity = -5.2}, "symplectic Euler step");
    expect_state_near(velocity_verlet_step(dt, initial_state, system),
                      State{.position = 2.54, .velocity = -5.108}, "velocity Verlet step");

    const PositionVerletState initialized = initialize_position_verlet(dt, initial_state, system);
    expect_near(initialized.current_position, 2.54, "position Verlet initialized current position");
    ...
    expect_near(stepped.current_position, 3.38, "position Verlet step current position");
    expect_near(stepped.previous_position, 3.0, "position Verlet step previous position");
    ...
    const PositionVerletState advanced = advance_position_verlet(dt, 1, initial_state, system);
