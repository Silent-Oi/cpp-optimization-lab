...
当前收尾顺序：**振子终止 → 理解自动向量化 → 简单多线程**。完成后进入 Project02；CUDA 在下一项目的 CPU 热扩散版本验证后引入。
...
理解编译器如何在条件允许时使用一条指令同时处理多个振子。已有 SoA 循环能够自动向量化时，重点是阅读诊断、理解生成代码和剩余元素的处理，不要求为此新增一个实现。
...
- 在有依据时简化循环或接口；
- 需要解释性能差异或确有代码改动时，复用现有 benchmark 进行对比。
...
在明确的数值容差下识别可以停止演化的振子，退出后续更新和新点绘制，并使已有拖尾自然结束。判据应考虑完整状态，避免在经过平衡位置时误判；明确无阻尼情况、重置及活跃状态的处理。

按需要观察终止前后的实际计算与绘制成本。带终止的近似计算与原有固定步数基线分别说明语义，避免把减少计算量误认为相同工作量下的实现加速。
...
├─ AoS update
├─ SoA update
├─ compiler-vectorized SoA update
└─ parallel update
...
4. 确认并测试编译器自动向量化；
5. 完成简单多线程实现和线程扩展性测试；
6. 固定步数的优化版本与参考实现满足约定数值容差，终止近似满足单独定义的判据；
7. 保存一组能够展示主要优化效果的最终 benchmark 结果；
8. 完成终止、活跃状态管理及相应显示行为。
